...
def run(model: str, face_model: str, num_hands: int,
        min_hand_detection_confidence: float,
        min_hand_presence_confidence: float, min_tracking_confidence: float,
        min_detection_confidence: float, min_suppression_threshold: float, camera_id: int, width: int, height: int) -> None:
  """Continuously run inference on images acquired from the camera.

  Args:
      model: Name of the gesture recognition model bundle.
	  face_model: Name of the TFLite face detection model.
  ...
  def hand_save_result(result: vision.GestureRecognizerResult,
  ...
  # Initialize the hand gesture recognizer model
  hand_base_options = python.BaseOptions(model_asset_path=model)
  hand_options = vision.GestureRecognizerOptions(base_options=hand_base_options,
                                          running_mode=vision.RunningMode.LIVE_STREAM,
                                          num_hands=num_hands,
                                          min_hand_detection_confidence=min_hand_detection_confidence,
                                          min_hand_presence_confidence=min_hand_presence_confidence,
                                          min_tracking_confidence=min_tracking_confidence,
                                          result_callback=hand_save_result)
  hand_recognizer = vision.GestureRecognizer.create_from_options(hand_options)
  
  # Initialize Mediapipe face detection
  def face_save_result(result: vision.FaceDetectorResult, unused_output_image: mp.Image, timestamp_ms: int):
  ...
  face_options = vision.FaceDetectorOptions(base_options=face_base_options,
                                          ...
                                          result_callback=face_save_result)
  ...
  while cap.isOpened():
    ...
    hand_recognizer.recognize_async(mp_image, time.time_ns() // 1_000_000)
    ...
    if recognition_frame is not None:
        if DETECTION_RESULT:
            cv2.imshow('face detection & gesture recognition', recognition_frame)
...
def main():
  ...
  run(args.model, args.face_model, int(args.numHands), args.minHandDetectionConfidence,
      args.minHandPresenceConfidence, args.minTrackingConfidence, args.minDetectionConfidence, args.minSuppressionThreshold, int(args.cameraId), args.frameWidth, args.frameHeight)
